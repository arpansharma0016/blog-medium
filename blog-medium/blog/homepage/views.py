...
def post(request, username, title, post_id):
    ...
    if not po.user_id == con.id:
        return redirect("author", username=username)
...
def author(request, username):
    ...
    posts = Post.objects.filter(user_id=con.id)
    for post in posts:
        like = Like.objects.filter(post_id=post.id).count()
        di[post] = like

    followers = Follow.objects.filter(followed=con.id).count()
    following = Follow.objects.filter(follower=con.id).count()
...
def like(request, username, post_id):
    po = Post.objects.filter(id=post_id).first()
    if not po:
        return JsonResponse({'status':'fail', 'message':'post 404', 'next':f'/{username}/'})
    conn = Connect.objects.filter(username=username).first()
    if not conn:
        return JsonResponse({'status':'fail', 'message':'user 404', 'next':'/'})
    if request.user.is_authenticated:
        con = Connect.objects.filter(username=request.user.username).first()
        if not con:
            return JsonResponse({'status':'fail', 'next':'/'})

        if not po.user_id == conn.id:
        ...
        messages.info(request, "You need to login first!")
        return JsonResponse({'status':'login', 'next':f'?next=/{username}/{slugify(po.title)}-{post_id}/'})
...
def bookmark(request, username, post_id):
    ...
    if request.user.is_authenticated:
        con = Connect.objects.filter(username=request.user.username).first()
        if not con:
            return JsonResponse({'status':'fail', 'next':f'/'})
...
def follow(request, username, post_id):
    ...
    if request.user.is_authenticated:
        ...
        curr_con = Connect.objects.filter(username=request.user.username).first()
        if con.id == curr_con.id:
            return JsonResponse({'status':'fail', 'message':'You cannot follow your account', 'next':f'/{username}/'})
...
def followers(request, username):
    ...
    usr = get_object_or_404(Me, user_id=con.id)
    posts = Post.objects.filter(user_id=con.id)
    fol = Follow.objects.filter(followed=con.id)
    following = Follow.objects.filter(follower=con.id)
    di = dict()
    for follow in fol:
        di[Connect.objects.filter(id=follow.follower).first()] = Me.objects.filter(user_id=follow.follower).first()
...
def following(request, username):
    con = get_object_or_404(Connect, username=username)
    usr = get_object_or_404(Me, user_id=con.id)
    fol = Follow.objects.filter(follower=con.id)
    follower = Follow.objects.filter(followed=con.id)
    posts = Post.objects.filter(user_id=con.id)
    di = dict()
    for follow in fol:
        di[Connect.objects.filter(id=follow.followed).first()] = Me.objects.filter(user_id=follow.followed).first()
