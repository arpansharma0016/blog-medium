...
from homepage.models import Like, Follow, Bookmark
import datetime
...
def me(request):
    if request.user.is_authenticated:
        ...
        followers = Follow.objects.filter(followed=con.id)
...
def followers(request):
    if request.user.is_authenticated:
        ...
        for fol in followers:
            di[Connect.objects.filter(id=fol.follower).first()] = Me.objects.filter(user_id=fol.follower).first()
        ...
        return render(request, "me_followers.html", context)

def remove_follower(request, user_id):
    if request.user.is_authenticated:
        curr_con = Connect.objects.filter(username=request.user.username).first()
        if not curr_con:
            return JsonResponse({'status':'fail', 'next':'/me/'})

        con = Connect.objects.filter(id=user_id).first()
        if not con:
            return JsonResponse({'status':'fail', 'next':'/me/'})

        if con.id == curr_con.id:
            return JsonResponse({'status':'fail', 'message':'You cannnot follow or unfollow your account', 'next':'/me/'})

        fol = Follow.objects.filter(follower=con.id).filter(followed=curr_con.id)
        if fol.exists():
            follow = fol.first()
            follow.delete()
        ...
        request.session['next'] = "me"
        messages.info(request, "You need to login first!")
        return JsonResponse({'status':'login', 'next':'?next=/me/'})
...
def following(request):
    if request.user.is_authenticated:
        ...
        for fol in following:
            di[Connect.objects.filter(id=fol.followed).first()] = Me.objects.filter(user_id=fol.followed).first()
        ...
        return render(request, "me_following.html", context)
