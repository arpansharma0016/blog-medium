    ...
    if not Confirm.objects.filter(username=uname).exists():
        ...
        if request.method == 'POST':
            ...
            if otp == old_otp:
                ...
                con.save()
                usr = Me.objects.create(user_id=con.id)
                usr.save()
...
def edit_post(request, post_id):
    if request.user.is_authenticated:
        ...
        po = Post.objects.filter(user_id=con.id).filter(id=post_id).first()
        if not po:
            messages.info(request, "No such Post Exists!")
            return redirect("me")
        ...
        request.session['next'] = "edit_post" + str(post_id)
        messages.info(request, "You need to Login First!")
        return redirect("login")
